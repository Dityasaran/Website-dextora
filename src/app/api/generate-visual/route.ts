...
async function generateImagen(prompt: string, accessToken: string, sceneId: string): Promise<string | null> {
    ...
    try {
        ...
        if (!res.ok) {
            console.warn(`Imagen request failed for ${sceneId}: ${res.statusText}`);
            return null;
        }
        ...
        if (!base64) {
            console.warn(`Imagen returned no image data for ${sceneId}`);
            return null;
        }
        ...
        console.warn(`Imagen generation error for ${sceneId}:`, err.message);
        return null;
    }
}
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!prompt || sceneId === undefined || sceneId === null) return NextResponse.json({ error: "Missing required fields" }, { status: 400 });

        const accessToken = await getAccessToken();

        // For reels, we MUST generate an image sequence (5 images) for the cinematic animated background
        if (isVertical) {
            console.log(`Generating 5 images for cinematic sequence: ${sceneId}`);
            const imagePromises = [1, 2, 3, 4, 5].map((idx) =>
                generateImagen(`${prompt}, cinematic angle ${idx}, varied perspective`, accessToken, `${sceneId}-${idx}`)
            );
            ...
            if (imageUrls.length === 0) {
                throw new Error("All image generation attempts for sequence failed.");
            }

            return NextResponse.json({ success: true, urls: imageUrls, type: "image" });
        }

        // Fallback for non-vertical (legacy) — Attempt Veo Generation First
